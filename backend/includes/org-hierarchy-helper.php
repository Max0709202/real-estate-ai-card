 ...
 *     「免許番号キーが一致する会社が ON」であることに加えて、次のいずれかを満たすこと:
 *       ① その会社に登録したログインメールに自分のメールが含まれている
 *       ② その会社の統括に指名されたマネージャー（店長）である
 *       ③ その会社の統括（全閲覧）に指名されている（このとき①へ自動で登録する）
 *     ②③は指名自体が運営または既存の統括の操作なので、メール登録を待たずに使える。
 ...
 *
 * 前提として、免許番号キーが一致する会社が ON になっていること。
 * そのうえで、次のいずれかに当てはまる方が使える:
 *   ① その会社に登録した「利用できるログインメール」に自分のメールが含まれている
 *   ② マネージャー（店長）に指名されている
 *   ③ 統括（全閲覧）に指名されている（このとき①へ自動で登録する）
 *
 * ②③は指名そのものが運営または既存の統括の操作なので、運営のメール登録を待たずに使える。
 * 会社が OFF のあいだは、どの指名を受けていても使えない。
 */
function orgHierarchyEnabledForUser(PDO $db, int $userId): bool
{
    ...
    $role = orgNormalizeRole($viewer['org_role']);
