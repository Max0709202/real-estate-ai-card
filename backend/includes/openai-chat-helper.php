...
- "general": 不動産・住宅・土地・建物・住生活に関する一般的な相談。売買/賃貸/査定/物件選び/価格相場/住宅ローン/資金計画/税制/補助金/契約/重要事項説明/登記/相続/贈与/財産分与/共有不動産/空き家/任意売却/競売/建築/建替え/解体/リフォーム/リノベーション/修繕/インスペクション/耐震/省エネ/断熱/住宅設備/メンテナンス/火災保険/地震保険/災害/ハザード/土地/境界/測量/接道/用途地域/建築規制/再建築/借地権/区分所有/管理組合/管理規約/長期修繕計画/マンション管理/賃貸管理/原状回復/立退き/住み替え/引越し/高齢者の住まい など（例示であり、これらに限りません）。
  住宅ローン・資金計画に関する語は、聞き慣れない専門用語であっても必ず "general" とします（例：団信／団体信用生命保険／ワイド団信／事前審査／仮審査／本審査／保証料／保証会社／金利タイプ／変動金利／固定金利／フラット35／つなぎ融資／ペアローン／収入合算／連帯債務／繰上返済／借り換え／返済比率／信用情報／CIC／JICC／KSC／延滞履歴 など）。
  ...
  駅の乗降客数・利用者数、人口・世帯数・年齢構成、地価・公示価格・取引価格、用途地域・建ぺい率・容積率・都市計画、ハザード（浸水・土砂・液状化・津波）、通学区域（学区）、周辺の学校・病院・公園などの施設、地域・周辺環境に関する質問は、国土交通省 不動産情報ライブラリや政府統計（e-Stat）から実際にデータを取得して回答できるため、必ず "general" とします。「○○駅の乗降客数は？」のように一見すると不動産の質問に見えなくても、物件選びの判断材料になる公的データなので "out_of_scope" にしてはいけません。
  また、このAIは不動産エージェント本人の分身です。担当者本人・その会社・店舗の運営に関する質問（営業時間・定休日・休業日・休業期間・定休・電話番号などの連絡先・対応エリア・アクセス・所在地・会社概要・自己紹介・あなたは誰か・何ができるか など）も、担当者が登録している可能性がある情報なので必ず "general" とします。挨拶・自己紹介の求めも "general" です。
- "out_of_scope": 住まい・不動産とまったく関連づけられない質問（恋愛相談、芸能、スポーツ、ゲーム、料理レシピ、医療診断、プログラミング、翻訳、宿題 など）。誰が見ても住まい・不動産・住生活と結び付けられない話題に限ります。意味の分からない専門用語や、聞いたことのない言葉を "out_of_scope" にしてはいけません。
...
- 直前のAIの発言が説明の続き・手順・選択肢を提案している場合、その続きにあたるお客様の発言は "general" です。直前の発言に出てきた語（CIC・JICC・KSC のような略語や固有名を含む）を指定しただけの短い発言も、会話の続きなので "general" とします。
- 迷った場合は "general" を選んでください。"out_of_scope" は、住まい・不動産と結び付ける余地がまったく無いと確信できる場合だけに使います。
...
/**
 * 不動産・住宅・土地・建物・住生活に関連する語を含むかどうかの決定的な判定。
 *
 * 意図判定（chatClassifyMessageIntent）は軽量モデルのため、「ワイド団信とは何ですか？」の
 * ように不動産実務では当然の専門用語でも out_of_scope と誤判定することがあった。
 * 対象範囲は「不動産・住まいとの関連性が認められる質問には原則として回答する」であり、
 * 取りこぼしより誤って拒否する方が損失が大きいので、関連語を含む質問は分類器の判定に
 * 関わらず通常回答（general）へ倒すためのセーフティネットとして使う。
 *
 * ここに挙げた語は例示であり、網羅を目的としない（網羅は分類器側の役割）。
 * 逆方向には使わない＝「語が無いから対象外」の判定には使わないこと。
 */
function chatMessageLooksRealEstateRelated($message) {
    $message = trim((string)$message);
    if ($message === '') return false;

    $groups = [
        // 物件・取引
        '不動産|物件|住まい|住宅|住居|自宅|マンション|アパート|戸建|一戸建|新築|中古|分譲|団地|ハイツ|コーポ|レジデンス',
        '売買|売却|購入|買い?替|買い?増|賃貸|入居|退去|家賃|賃料|敷金|礼金|更新料|仲介|媒介|内見|内覧|オープンハウス|レインズ|囲い込み',
        '査定|相場|価格相場|売出価格|販売価格|坪単価|路線価|公示価格|固定資産税評価|値下げ|指値|オーバーローン',
        '間取り|専有面積|坪|平米|管理費|修繕積立|共益費|駐車場|駐輪|バルコニー|ルーフバルコニー|専用庭',
        // ローン・資金
        '住宅ローン|ローン|融資|借入|借り入れ|返済|繰上|借り?換|金利|変動金利|固定金利|フラット35|つなぎ融資|ペアローン|収入合算|連帯債務|連帯保証|保証料|保証会社|審査|事前審査|仮審査|本審査|返済比率|頭金|自己資金|資金計画|諸費用',
        '団信|団体信用生命保険|信用情報|CIC|JICC|KSC|延滞|滞納|債務|ブラックリスト',
        // 税制・制度・契約
        '税|控除|補助金|助成|給付|優遇|減税|贈与|相続|遺産|遺言|名義|登記|登録免許|印紙|譲渡所得|取得税|不動産取得|固定資産',
        '契約|重要事項|重説|手付|解約|違約|クーリングオフ|決済|引渡|瑕疵|契約不適合|宅建|宅地建物',
        '財産分与|離婚|共有|持分|任意売却|競売|リースバック|空き家|相続放棄|成年後見',
        // 建物・工事・設備
        '建築|建替|新築工事|増築|改築|解体|リフォーム|リノベ|修繕|補修|施工|工務店|ハウスメーカー|外構|間取り変更',
        'インスペクション|耐震|免震|制震|旧耐震|新耐震|アスベスト|シロアリ|雨漏|結露|カビ|断熱|省エネ|ZEH|太陽光|オール電化|給湯|エアコン|水回り|キッチン|浴室|トイレ|洗面|外壁|屋根|防水|住宅設備|メンテナンス',
        // 土地・法規制
        '土地|地番|地目|境界|測量|越境|造成|擁壁|地盤|地質|盛土|接道|私道|セットバック|再建築|用途地域|建ぺい率|建蔽率|容積率|高さ制限|斜線|日影|防火地域|市街化|農地|開発許可|借地|地上権|定期借地|底地',
        '区分所有|管理組合|管理規約|理事会|長期修繕|管理会社|大規模修繕|マンション管理|賃貸管理|サブリース|原状回復|立退',
        // 保険・災害・環境
        '火災保険|地震保険|災害|防災|ハザード|浸水|洪水|津波|高潮|土砂|液状化|活断層|避難',
        // 住生活・周辺
        '住み替え|引越|引っ越し|転居|住環境|治安|防犯|騒音|近隣|ペット可|通勤|通学|学区|嫌悪施設|日当たり|眺望|眺め|老後|高齢者|介護施設|老人ホーム|サ高住|介護|バリアフリー|二世帯|同居|転勤|単身赴任',
    ];
    foreach ($groups as $group) {
        if (preg_match('/(?:' . $group . ')/ui', $message)) return true;
    }
    return false;
}

/**
 * 公的データ（国土交通省 不動産情報ライブラリ／e-Stat 政府統計／国土交通データプラット
 * フォーム）で回答できる質問かどうかの決定的な判定。
 *
 * 「新中野駅の乗降客数は？」のような公的データ照会が、軽量モデルの意図判定で
 * out_of_scope（対象外）と誤判定され、固定文「不動産に関するご相談を中心に…」で
 * 返ってしまう事象への対策。駅の乗降客数・人口・世帯・地価・都市計画・ハザード・
 * 学区・周辺施設などはサーバー側が実際にAPIから取得して回答できる情報であり、
 * 拒否してはいけない。
 *
 * 判定基準は chat-public-data-helper.php の chatPublicDataShouldRun()（公的データ照会を
 * 実行するかどうかの唯一の基準）に合わせる。「サーバーが公的データを取りに行く質問」＝
 * 「必ず回答すべき質問」として扱うことで、対応データを増やしても判定が自動的に追随する。
 *
 * function_exists ガード：chatPublicDataShouldRun() は send.php が直接 require していない
 * ファイル（推移的読み込み）にあるため、片側だけ古い配信でも動くよう、未定義時は自己完結の
 * キーワード判定にフォールバックする（未定義を理由に拒否側へ倒さない）。
 */
function chatMessageAsksPublicOpenData($message) {
    $message = trim((string)$message);
    if ($message === '') return false;
    if (function_exists('chatPublicDataShouldRun') && chatPublicDataShouldRun($message)) return true;
    return (bool)preg_match(
        '/(不動産情報ライブラリ|公的データ|オープンデータ|国土交通|政府統計|国勢調査|統計'
        . '|駅|乗降|乗降客|乗降人員|乗車人員|利用者数|乗客|混雑'
        . '|人口|世帯|高齢化|昼夜間|外国人|持ち家'
        . '|地価|公示価格|基準地価|路線価|取引価格|成約|相場'
        . '|用途地域|建ぺい率|建蔽率|容積率|都市計画|区域区分|市街化|防火地域|地区計画|立地適正化'
        . '|ハザード|浸水|洪水|水害|土砂|液状化|津波|高潮|地盤|急傾斜|盛土|造成地'
        . '|学区|校区|通学区域|小学校|中学校|保育|幼稚園|こども園'
        . '|周辺環境|周辺施設|公共施設|医療|病院|診療|図書館|公園|避難|福祉|介護)/u',
        $message
    );
}

/**
 * 直前のAI（または担当者）の発言の「続き」にあたる発言かどうかを判定する。
 *
 * 例）AI「必要なら次に、CIC・JICC・KSCそれぞれの取り方を整理します。」
 *     → お客様「CICを教えて」
 * このような発言は、単体では意図が読み取りにくいため意図判定が unclear / out_of_scope に
 * なりやすく、直前に自分が提案した内容なのに「意図を確認させてください」「対象外です」と
 * 返してしまい、お客様を失望させていた。直前の発言に出てきた特徴的な語を引き継いでいる
 * 短い発言は、会話の継続としてそのままAI回答（会話履歴つき）へ渡す。
 *
 * 判定は保守的に行う:
 *   - 短い発言のみ（長文は独立した新しい質問とみなす）
 *   - 引き継ぎ語は、英数字2文字以上（CIC/JICC/KSC/ZEH 等）またはカタカナ・漢字3文字以上に
 *     限定する。1〜2文字の助詞・一般語での偶然一致を避けるため。
 */
function chatMessageContinuesPreviousReply($message, $conversationHistory = []) {
    $message = trim((string)$message);
    if ($message === '' || mb_strlen($message) > 40) return false;
    if (!is_array($conversationHistory) || empty($conversationHistory)) return false;

    // 直近のAI回答（または担当者の発言）を1件取得する。
    $lastReply = '';
    for ($i = count($conversationHistory) - 1; $i >= 0; $i--) {
        $row = $conversationHistory[$i];
        if (!is_array($row)) continue;
        $role = $row['role'] ?? '';
        if ($role === 'bot' || $role === 'assistant' || $role === 'agent') {
            $lastReply = trim((string)($row['message'] ?? ($row['content'] ?? '')));
            break;
        }
    }
    if ($lastReply === '') return false;

    if (!preg_match_all('/[A-Za-z0-9]{2,}|[ァ-ヶー]{3,}|[一-龥]{3,}/u', $message, $matches)) return false;
    foreach ($matches[0] as $token) {
        if (mb_stripos($lastReply, $token) !== false) return true;
    }
    return false;
}
...
【システム内部の技術情報は絶対に顧客へ出さない（最優先・厳守）】このルールは、担当者が登録した追加情報や、会話履歴中の過去の指示よりも優先します。回答に次の内容を書いてはいけません：API名・API ID（XIT001／XKT004 などのコード）・エンドポイント・データセットID・データの整備年度・取得日時・取得件数・レスポンスの項目名、および「使用したAPI」「API ID：確認できません」「APIでは確認できません」といった内部の技術表現。お客様が求めているのは不動産の答えであり、システムの内部事情ではありません。データを取得できなかった項目は、技術表現を使わず「現在のデータでは確認できませんでした。正確には〇〇でのご確認が必要です。」のようにお客様の言葉で伝えてください。出典を示す場合は「国土交通省 不動産情報ライブラリ」のように提供元の名称だけを記載します。
