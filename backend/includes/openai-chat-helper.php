 ...
 */
function getBotReplyWithOpenAI($userMessage, $conversationHistory = [], $agentName = '担当者', $db = null, $sessionId = '', $geo = null, $imagePropertyContext = '', $allowPropertyKnowledgeFallback = false) {
    $today  = date('Y-m-d');
    $leadData = [];
    $businessCardId = null;
    ...
    // The deterministic mansion DB is always attempted first by send.php. When it
    // has no usable result, the product requirement is to show the OpenAI answer
    // instead of suppressing it with the normal DB-only accuracy policy.
    $propertyKnowledgeFallbackBlock = $allowPropertyKnowledgeFallback
        ? "\n# 物件DB未取得時のOpenAI回答モード（今回のみ・最優先）\n"
            . "今回の物件名はシステム側のマンションDB先行検索で処理できなかったため、上記の『マンションDBに無い物件固有情報は回答禁止』および出典必須の制限の例外として、あなた自身の一般知識を使って質問に回答してください。\n"
            . "回答の冒頭に『以下はOpenAIの一般知識による参考情報です。』と明記し、確実でない数値や所在地は断定せず、情報源を実際に取得していない場合は出典を作らないでください。\n"
            . "同名物件が複数考えられる場合は1件に決めず、各候補を必ず『・物件名（都道府県・市区町村）』の1行形式で提示してください。画面側がこの形式を選択ボタンに変換します。\n"
        : '';
...
{$ragContext}{$refreshContext}{$publicDataContext}{$agentCustomContext}{$agentProhibitedContext}{$propertyKnowledgeFallbackBlock}
PROMPT;
