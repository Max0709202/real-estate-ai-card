    ...
    if (!empty($intake['handled'])) {
        ...
        if ($directMansionAnswer !== null) {
            ...
            if ($isUnresolvedMansionLookup) {
                ...
                // 固定文だけだとお客様が次に何をすればよいか分からず会話が止まるため、
                // 所在地をいただければ公的データ（用途地域・ハザード等）で回答できることを案内する。
                // ※この文面は全国マンションDBに該当が無かったことのみを意味し、
                //   不動産情報ライブラリAPIの障害を意味しない（APIはこの時点で未照会）。
                $reply = "該当するマンションが見つかりませんでした。正式名称や所在地などをご確認ください。\n\n"
                    . "所在地（例：埼玉県川口市弥平2-20-3）をお知らせいただければ、用途地域・建ぺい率・容積率やハザード情報、通学区域、周辺施設などを公的データからお調べできます。";
                $sources = [];
                $result = null;
