...
<body>
            ...
            <div class="edit-main">
                ...
                <div id="company-profile-section" class="edit-section">
                    ...
                    <form id="company-profile-form" class="edit-form">
                        ...
                        $prefectures = [
                            '信託会社等', '国土交通大臣',
                            '北海道知事 石狩', '北海道知事 渡島', '北海道知事 檜山', '北海道知事 後志',
                            '北海道知事 空知', '北海道知事 上川', '北海道知事 留萌', '北海道知事 宗谷',
                            '北海道知事 オホ', '北海道知事 胆振', '北海道知事 日高', '北海道知事 十勝',
                            '北海道知事 釧路', '北海道知事 根室',
                            '青森県知事', '岩手県知事', '宮城県知事', '秋田県知事', '山形県知事', '福島県知事',
                            '茨城県知事', '栃木県知事', '群馬県知事', '埼玉県知事', '千葉県知事', '東京都知事', '神奈川県知事',
                            '新潟県知事', '富山県知事', '石川県知事', '福井県知事', '山梨県知事', '長野県知事', '岐阜県知事',
                            '静岡県知事', '愛知県知事', '三重県知事', '滋賀県知事', '京都府知事', '大阪府知事', '兵庫県知事',
                            '奈良県知事', '和歌山県知事', '鳥取県知事', '島根県知事', '岡山県知事', '広島県知事', '山口県知事',
                            '徳島県知事', '香川県知事', '愛媛県知事', '高知県知事', '福岡県知事', '佐賀県知事', '長崎県知事',
                            '熊本県知事', '大分県知事', '宮崎県知事', '鹿児島県知事', '沖縄県知事'
                        ];
                        ?>
                        <div class="form-section">
                    ...
                    <form id="template-form-edit" class="edit-form">
                            ...
                            <div id="card-header-template-grid-edit" class="template-grid" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 12px;">
                                <?php for ($i = 1; $i <= 10; $i++): $path = "assets/images/card-header ({$i}).jpg"; ?>
                                    <label class="template-tile" style="border:1px solid #e2e8f0; border-radius:10px; overflow:hidden; cursor:pointer; background:#fff; position: relative;">
                                        <!-- iOS Safari: label toggle can fail when display:none. Keep input in layout but visually hidden. -->
                        ...
                        <div class="form-section">
                            ...
                            <div class="upload-area" id="card-header-bg-upload-edit" data-upload-id="card_header_bg">
                                ...
                                <input type="file" id="card_header_bg_file_edit" accept="image/*" style="position:absolute; opacity:0; width:1px; height:1px; pointer-events:none;">
  ...
  cancelBtn.addEventListener('click', async function () {
    ...
    // ✅ Use modal if available, fallback if not (利用停止関連のため確認ボタンは「利用停止する」)
      ...
      window.showConfirm(confirmMessage, async () => {
        window.showConfirm(
          ...
          '即座に利用停止',
          {
            ...
            onSecondary: async () => { await processCancellation(false); },
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (headerGreetingForm) {
                ...
                headerGreetingForm.addEventListener('submit', async function(e) {
                    ...
                    const logoFileInput = document.getElementById('company_logo');
                    if (logoUploadArea) {
                        ...
                        } else if (logoFileInput && logoFileInput.files && logoFileInput.files[0]) {
                            // Upload file directly from input (e.g., if cropper was cancelled)
                            try {
                                const file = logoFileInput.files[0];
                                data.company_logo = await uploadImage(file, 'logo');
                                logoUploadArea.dataset.uploadedPath = data.company_logo;
                            } catch (error) {
                                console.error('Logo upload error:', error);
                                showError('ロゴのアップロードに失敗しました: ' + error.message);
