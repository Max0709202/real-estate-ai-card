...
<body>
    <div class="reset-email-container">
        <div class="reset-email-box">
            <h1>メールアドレスリセット</h1>
            <p>新しいメールアドレスを入力してください。<br>確認用のリンクを新しいメールアドレスに送信します。</p>
            ...
            <form id="reset-email-form">
                <div class="form-group">
                    ...
                    <input type="email" id="new_email" name="new_email" class="form-control" required placeholder="newemail@example.com">
                </div>
                <button type="submit" class="btn-primary" id="submit-btn">確認メールを送信</button>
            </form>
    ...
    <script>
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('../backend/api/auth/reset-email.php', {
                    method: 'POST',
                    headers: {
                ...
                if (result.success) {
                    showMessage(result.message || '確認メールを新しいメールアドレスに送信しました。メールボックスをご確認ください。', 'success');
                    form.reset();
                } else {
